...
import { formatDate, formatLastCheckIn, pct } from "@/lib/formatting";
    ...
    <>
      ...
      <div className="flex-1 space-y-5 overflow-y-auto p-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <MiniStat label="Adherence" value={pct(player.adherencePercent)} />
        </div>
        ...
        <div>
          ...
          <p className="mt-2.5 text-xs text-muted">
            ...
            {formatDate(player.nextSession.date)} · Last check-in{" "}
        ...
        <details className="rounded-card border border-hairline bg-surface-2 px-3 py-2 text-sm">
          ...
          <dl className="mt-2 grid grid-cols-3 gap-2 text-xs">
            ...
            <Detail label="Load (week)" value={`${player.weeklyLoad}`} />
            <Detail label="Planned" value={`${player.plannedLoad}`} />
          </dl>
        </details>
