...
| **S11** | **`player_status` values are self-attested** by the player's client. | **HIGH (integrity)** | **DONE (safety fields)** (migration 20260708, staging) — a BEFORE trigger overrides injury_status (from `injuries`) + readiness (from logged `daily_metrics.readiness_score`) with server truth; soft trend metrics stay engine-computed + clamped. A player can no longer fake availability/readiness. Full-engine server derivation of acwr/adherence is a future Edge-Function option (avoids EWMA SQL drift). |
...
| **S13** | **`next@14` carried HIGH XSS advisories** (apps/web). | **HIGH (pre-ship)** | **DONE** — bumped to next@16 (accepts React 18, no React-19 migration); 14 HIGH/CRITICAL advisories cleared (2 moderate remain); `middleware.ts`→`proxy.ts` (Next 16 convention); typecheck + build green. |
