...
export const ENTRIES = [
  ...
  {
    id: 'load.aerobic.trimp',
    rule: 'Aerobic session load via Banister TRIMP: HRr = clamp((avgHr − restHr)/(maxHr − restHr), 0, 1); weight(HRr) = 0.64·e^(1.92·HRr) (male) / 0.86·e^(1.67·HRr) (female); TRIMP = duration_min × HRr × weight. HRmax is estimated from age (Tanaka 2001: 208 − 0.7·age) when no observed max is available; HRrest defaults to a population 60bpm when no observed resting HR is available (that default lowers confidence — per-individual HRrest matters more than HRmax at typical training intensities). Falls back to a duration proxy (round(min×3), the existing workoutLoad heuristic) when avgHr or a usable HRmax/HRrest pair is unavailable.',
    value: { maleC: 0.64, maleK: 1.92, femaleC: 0.86, femaleK: 1.67, hrMaxIntercept: 208, hrMaxAgeSlope: 0.7, restHrDefault: 60 },
    evidenceLevel: 'L2',
    source: 'Banister 1991; Morton, Fitz-Clarke & Banister 1990 (impulse-response); Tanaka 2001 (HRmax=208−0.7·age)',
    confidence: 'moderate',
  ...
  {
    id: 'load.form.model',
    rule: '"Form" (CTL/ATL/TSB) via the TrainingPeaks Performance Manager EWMA: CTL (fitness, 42-day time constant) and ATL (fatigue, 7-day time constant) each roll forward day-by-day as CTL = CTL_prev·e^(−1/ctlDays) + load·(1−e^(−1/ctlDays)) (ATL analogous with atlDays), seeded at 0 from the first loaded day; TSB (form) = CTL − ATL. TSB ≥ freshTsb ⇒ band "fresh"; TSB ≤ fatiguedTsb ⇒ "fatigued"; else "neutral". Fewer than minDays loaded days ⇒ band null (too little history to trust the read). Confidence scales 0→1 with loaded-day count up to matureDays.',
    value: { ctlDays: 42, atlDays: 7, freshTsb: 5, fatiguedTsb: -15, minDays: 14, matureDays: 42 },
    evidenceLevel: 'L4',
    source: 'TrainingPeaks Performance Manager / Coggan (CTL 42d / ATL 7d EWMA); Banister impulse-response 1991',
    confidence: 'low',
    lastReviewed: '2026-07-20',
    appliesTo: ['load']
  },
  {
    id: 'load.form.dose',
    rule: 'D9 dose-shrink: when the athlete\'s form band (load.form.model) reads "fatigued" (high accumulated aerobic fatigue), gently trim the current week\'s gym VOLUME by fatiguedVolumeMult — a soft autoregulation nudge, not a deload. Combined with the existing recovery/load multiplier via Math.min (never Math.multiply) so a deload that has already cut volume below this floor is not double-counted; the trim only bites in the "fatigued but not already deloading" middle zone.',
    value: { fatiguedVolumeMult: 0.9 },
    evidenceLevel: 'L4',
    source: 'Autoregulation — reduce gym volume under high concurrent aerobic load (Rønnestad & Mujika 2014 concurrent-training interference; standard autoregulation practice)',
    confidence: 'low',
