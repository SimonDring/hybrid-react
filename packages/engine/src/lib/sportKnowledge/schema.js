...
export function validateSportProfile(p) {
  ...
  // ── §6 positions — a position MAY declare `priorityPatterns` (Sprint 3 B2): a small D11
  //     selection nudge toward the movement patterns this position prioritises. Validated only
  //     where authored (a missing list is a valid scaffold). Each entry must be a real catalogue
  //     `pattern` value, or the nudge would silently never fire. ─────────────────────────────────
  if (isArr(p.positions)) {
    p.positions.forEach((pos, i) => {
      if (isObj(pos) && pos.priorityPatterns != null) {
        const lbl = `${id}.positions[${i}].priorityPatterns`;
