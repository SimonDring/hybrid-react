...
| `20260706_security_hardening.sql` | 2026-07-05 | Security hardening: S2 token-column lockdown (column-level grants on `wearable_connections`, tokens browser-invisible), S3 NOT VALID bounds (injuries free-text, `users.profile` size cap, team free-text), S6 explicit `delete_user()` completeness (+ team surfaces), S9 no-rejoin-after-removal guard, S10 `handle_new_user()` search_path pin | **Yes** — applied to prod by 2026-07-16 (Simon's cumulative `db push` completed all pending) |
| `20260707_oauth_state.sql` | 2026-07-05 | S1 (CRITICAL): signed OAuth `state` — `oauth_states` nonce table (RLS on, no client policies) + `issue_oauth_state()` (authenticated) / `consume_oauth_state()` (service_role-only) RPCs | **Yes** — applied to prod by 2026-07-16 (Simon's cumulative `db push` completed all pending) |
| `20260708_player_status_integrity.sql` | 2026-07-05 | S11: server-authoritative coach board — `derive_injury_status()` / `latest_readiness()` + `player_status_server_truth()` BEFORE trigger overrides injury_status/readiness with server truth and clamps the soft trend metrics | **Yes** — applied to prod by 2026-07-16 (Simon's cumulative `db push`) |
| `20260709_player_status_identity.sql` | 2026-07-05 | Coach-board identity: `display_name` column on `player_status` (+ length check) + `player_display_name()`; extends the server-truth trigger to stamp the name server-side | **Yes** — applied to prod by 2026-07-16 (Simon's cumulative `db push`) |
| `20260710_team_join_codes.sql` | 2026-07-05 | Team founding + invites via join codes: `join_code` column + partial unique index on `teams`, `gen_join_code()` / `create_team()` / `join_team_with_code()` / `rotate_team_code()` SECURITY DEFINER RPCs | **Yes** — applied to prod by 2026-07-16 (Simon's cumulative `db push`) |
...
| `20260712_player_status_membership_scope.sql` | 2026-07-10 | F3 (2026-07-09 data review, TEAM DATA ISOLATION): an ENDED membership ends the coach's read — `coach_reads_member()` helper + re-scoped `player_status` SELECT policy; `team_members_cleanup_status` trigger deletes the derived row on roster DELETE / status→'left'; one-time orphan backfill. Depends on `20260711_team_scoping.sql` (applied to prod 2026-07-06; see SECURITY-DEPLOY) | **Yes** — applied 2026-07-14 (Simon), per the P1-10 runbook (`supabase/SECURITY-DEPLOY.md`) |
| `20260713_m5_outcomes_substrate.sql` | 2026-07-15 | M5 outcomes substrate (design `docs/design/m5-substrate/SCHEMA-AND-PRIVACY.md`, 🔒 6): 12 new tables — nine owner-private append-only (`block_outcomes`, `session_outcomes`, `readiness_snapshots`, `monitoring_entries`, `test_results`, `match_performances`, `external_load_observations`, `baselines`, `served_artefacts`), the cross-person derived `squad_signal_snapshots` (no raw-vital column; SELECT needs active membership AND active consent), and the consent pair (`consent_grants` + `consent_events`, RLS enabled — B1). `has_active_grant()` predicate (C2 oracle-locked), append-only guard + snapshot server-truth (C1) + consent-audit triggers, extended `delete_user()` covering all 12 tables (C3). Revocation is POLICY-ONLY (🔒 D1 — athlete keeps their history; no delete-own-history trigger). Depends on the team spine (`coach_reads_member`, `is_coach_of_team`, `derive_injury_status`/`latest_readiness`). | **Yes** — applied to prod 2026-07-16 (Simon), staging→`rls-harness-m5.mjs` green→prod per the M5 runbook |
